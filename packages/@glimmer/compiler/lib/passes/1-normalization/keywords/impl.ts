...
import { isKeywordOrReserved, KEYWORDS_TYPES, RESERVED_WORDS } from './mapping';
...
export class Keywords<K extends KeywordType, KeywordList extends Keyword<K> = never>
  ...
  translate(
    ...
    if (path && path.ref.type === 'Free' && isKeywordOrReserved(path.ref.name)) {
      let { name } = path.ref;

      if (name in RESERVED_WORDS) {
        let alternative = RESERVED_WORDS[name];

        return Err(
          generateSyntaxError(
            `\`${name}\` is a reserved word, which is set aside for future use, but has not been implemented yet. Consider using the alternative version, \`${alternative}\``,
            node.loc
          )
        );
      }
